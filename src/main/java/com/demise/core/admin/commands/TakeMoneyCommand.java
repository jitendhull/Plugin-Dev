...
public class TakeMoneyCommand implements CommandExecutor {
    ...
    public TakeMoneyCommand(JavaPlugin plugin, EconomyService economyService, ConfigManager configManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length != 2) {
            MessageUtils.send(sender, configManager.getPrefix(), "Usage: /takemoney <player> <amount>");
            return true;
        }
        ...
        economyService.takeBalance(target.getUniqueId(), targetName, amount, actorId)
                .thenAccept(result -> TaskUtils.runSync(plugin, () -> {
                    if (!result.success()) {
                        MessageUtils.send(sender, configManager.getPrefix(), "Player does not have enough funds.");
                        return;
                    }
                    MessageUtils.send(sender, configManager.getPrefix(), "Took " + amount + " from " + targetName + ".");
                }))
                .exceptionally(exception -> {
                    TaskUtils.runSync(plugin, () -> MessageUtils.send(sender, configManager.getPrefix(), "Failed to take money."));
                    return null;
                });
        return true;
    }
