...
public class BanCommand implements CommandExecutor {
    ...
    public BanCommand(JavaPlugin plugin, ModerationService moderationService, ConfigManager configManager) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length < 1) {
            MessageUtils.send(sender, configManager.getPrefix(), "Usage: /ban <player> [duration] [reason]");
            return true;
        }
        ...
        if (args.length >= 2) {
            ...
            if (parsed.valid()) {
                ...
                duration = DurationParser.parse(configManager.getDefaultBanDuration());
                reasonStartIndex = 1;
            }
        } else {
            duration = DurationParser.parse(configManager.getDefaultBanDuration());
        ...
        String reason = reasonStartIndex < args.length ? String.join(" ", java.util.Arrays.copyOfRange(args, reasonStartIndex, args.length)) : "";
        if (reason.isBlank()) {
        ...
        moderationService.ban(target.getUniqueId(), actorId, finalReason, expiresAt)
                .thenRun(() -> TaskUtils.runSync(plugin, () -> {
                    MessageUtils.send(sender, configManager.getPrefix(), "Banned " + targetName + ".");
                    ...
                    if (onlineTarget != null) {
                        ...
                        onlineTarget.kickPlayer(MessageUtils.color(configManager.getPrefix() + message));
                    }
                }))
                .exceptionally(exception -> {
                    TaskUtils.runSync(plugin, () -> MessageUtils.send(sender, configManager.getPrefix(), "Failed to ban player."));
                    return null;
                });
        return true;
    }
